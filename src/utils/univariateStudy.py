...
class univariateStudy:
    ...
    def save_graph(self):
        ...
        output = f'axis_x="{self.axis_x}" , filters={self.chosen_filters}, plot_type="{self.plot_type}" , '\
        + 'default_values={'\
    ...
    def __set_axis(self):
        ...
        logger.debug("Axes définis: axis_x=%s", axis_x)
        return axis_x
    ...
    def get_data_points_ingredients(self, df, axis_x, range_axis_x, chosen_filters, range_filters):
        columns = [axis_x] + chosen_filters
        if "recipe_id" in self.dataframe.columns:
            columns += ["recipe_id"]
        df = df[columns]
    ...
    def __draw_graph(self, x, y, recipes_id):
        ...
        with col[1]:
            ...
            elif self.plot_type == "bar_ingredients" or self.plot_type == "bar_techniques":
