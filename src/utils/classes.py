...
class Study:
    ...
    def get_data_points(
        ...
        columns = [axis_x, axis_y, "id"] + [
            filtre
            for filtre in chosen_filters
            if (filtre != axis_x and filtre != axis_y)
        ]
            ...
            with st.expander(label=self.key, expanded=True):
                ...
                with st.form(self.key):
                    if st.form_submit_button(label="Draw graph"):
                        x, y, recipes_id = self.get_data_points(
                            self.dataframe,
                            self.axis_x,
                            self.axis_y,
                            self.range_axis_x,
                        ...
                        display_df = self.dataframe[
                            self.dataframe["id"].isin(recipes_id)
                        ]
                        display_df = display_df.sort_values(
                            by="count_total", ascending=False
                        )[:10]
                    ...
                    if st.form_submit_button(label="Delete graph"):
                        self.delete = True
                        logger.info(
                            "Graphique supprimé pour l'instance avec key='%s'", self.key
                        )
                        st.rerun()
