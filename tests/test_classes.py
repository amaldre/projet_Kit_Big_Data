...
def test_study_get_data_points():
    ...
    expected_df = df[
        (df[axis_x] >= range_axis_x[0]) & (df[axis_x] <= range_axis_x[1]) &
        (df[axis_y] >= range_axis_y[0]) & (df[axis_y] <= range_axis_y[1]) &
        (df['filter_col'] >= range_filters[0][0]) & (df['filter_col'] <= range_filters[0][1])
    ]
...
def test_study_display_graph():
    ...
    with patch('streamlit.expander'), \
         patch('streamlit.selectbox', side_effect=['axis_x', 'axis_y']), \
         patch('streamlit.slider', return_value=(1, 3)), \
         patch('streamlit.form'), \
         patch('streamlit.form_submit_button', side_effect=[True, False]), \
