...
def test_main(mock_st):
    ...
    mock_st.button.side_effect = [
        False,
        False,
    ]  # Mock des boutons : aucun bouton cliqué
    ...
    mock_st.columns.assert_called_once_with(
        2
    )  # Vérifie si les colonnes ont été définies
    mock_st.button.assert_any_call(
        "Ajout graphe univarié"
    )  # Vérifie le bouton univarié
...
def test_main_error_handling(mock_st):
    ...
    with patch(
        "src.pages.DataViz.UnivariateStudy", side_effect=Exception("Test Error")
    ):
        main()
